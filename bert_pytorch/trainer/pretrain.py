...
# from bert_pytorch.model.language_model import BERTLM_2
        ...
        if train==2:
            ...
            for i, data in data_iter:
                ...
                if i>0:
                    ...
                    prev=x
            torch.save(prev,'/home/sriharshkamma/final/BERT-pytorch/tensor-pytorch_embed.pt') 
...
class BERTTrainer_embed:
    ...
    def __init__(self, bert: BERT, vocab_size: int,
        ...
        # Initialize the BERT Language Model, with BERT model
        if restore_model_path==None:
            self.model = BERTLM(bert, vocab_size,0).to(self.device)
        else:
            self.model=torch.load(self.restore_model_path)
            self.model.to(self.device)
    ...
    def iteration(self, epoch, data_loader, train=1):
        ...
        if train ==1 or train ==0:
            for i, data in data_iter:
                ...
                # 1. forward the next_sentence_prediction and masked_lm model
                _,mask_lm_output = self.model.forward(data["bert_input"], data["segment_label"])
